...
export const requiredString = (label: string) => z.string().trim().min(1, `${label} không được để trống`);

export const tourGroupSchema = z.object({
  groupName: requiredString('Tên đoàn'),
  contactName: requiredString('Người liên hệ'),
  phone: requiredString('Số điện thoại').regex(/^[0-9+\-\s]{8,}$/, 'Số điện thoại không hợp lệ'),
...
export const postSchema = z.object({
  ...
  slug: requiredString('Slug').regex(/^[a-z0-9-]+$/, 'Slug chỉ gồm chữ thường, số và dấu gạch ngang'),
